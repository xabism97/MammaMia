...
ajaxRequest.onreadystatechange = function () {
    ...
    if(ajaxRequest.readyState == 4){
        if(ajaxRequest.status == 200){
          ...
          createList("mammamiaapp.tipomasa", messagesArray);
...
function createList(name, array) {
  ...
  for (let i = 0; i < array.length; i++) {
    if (array[i].model == name) {
      ...
      textoPizzas.textContent = "Pizzas que contienen esta masa:\n";
      listaPizzas.append(textoPizzas);
      //ul.appendChild(textoPizzas);

      for (var y = 0; y < array.length; y++) {
        if ((array[y].model == "mammamiaapp.pizza") && (array[y].fields.tipoMasa == array[i].pk)) {
